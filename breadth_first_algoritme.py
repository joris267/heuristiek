...
def layIntersectingPaths():
    ...
    print 'intersection paths : ', intersection_paths
...
def isBetterPath(new_intersections, old_path_length, new_path_length, total_paths, iterations):
    ...
    """
    if len(new_intersections) == 0 or len(new_intersections) == 1:
        return True
    ...
    # percentage = float(numpy.abs(float(numpy.cos(delta/ (float(cross_value)*weight)))))
    percentage = float(numpy.exp((0.2 * delta)-0.003*iterations))/float((cross_value**0.8))
...
def simulatedAnnealing(intersecting_paths, paths_dict, max_iteration=1000):
    ...
    while len(intersecting_paths) != 0:
        ...
        current_path_length = len(paths_dict_new[current_path_number-1])
        ...
        if isBetterPath(new_intersections, current_path_length, len(new_path), len(paths_dict_new), counter):
            ...
            print "new_intersections ", len(new_intersections), "number of paths ", len(paths_dict_new) - len(intersecting_paths) #, new_intersections
